...
#include "skialin/node/Lighting.h"
...
// Ported from skiko's org.jetbrains.skiko.node.RenderNode (skiko/src/commonMain/cpp/common/node/RenderNode.cpp),
// which is itself adapted from frameworks/base/libs/hwui/RenderNode.{h,cpp}. Unlike a plain
// SkPicture-recording layer, this is an SkDrawable: when one RenderNode's recording embeds
// another (a nested Compose GraphicsLayer), the embedded node stays a *live* reference rather
// than a frozen snapshot of whatever it looked like at record time, because SkCanvas::drawDrawable
// replays the referenced SkDrawable::draw() fresh on every playback instead of inlining a fixed
// picture. The dependency/observer bookkeeping below exists only to decide when it's safe to
// cache ("snapshot") a subtree as a plain SkPicture for speed without losing that liveness.
...
public:
    ...
    void setClipRect(const std::optional<SkRect>& clipRect, SkClipOp op, bool doAntiAlias);
    void setClipRRect(const std::optional<SkRRect>& clipRRect, SkClipOp op, bool doAntiAlias);
    void setClipPath(const std::optional<SkPath>& clipPath, SkClipOp op, bool doAntiAlias);
