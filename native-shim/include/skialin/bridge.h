 ...
 * skialin_bridge_GraphiteBackendTexture_MakeD3D12Resource; those resources must be released before
 * *outKeepAlive is deleted, since the device they live on goes with it. Null on failure. */
 ...
 * wgpu::TextureUsage values (see DawnGraphiteTypes.h / webgpu.h), not DXGI_FORMAT. Dawn only imports
 * resources created with D3D12_RESOURCE_FLAG_ALLOW_SIMULTANEOUS_ACCESS; import failures are logged
 * to stderr (Dawn's uncaptured-error callback) before this returns null.
 *
