...
class RRect internal constructor(
    ptr: Long,
) : Managed(ptr, RRectNative::nRelease) {
    ...
    fun inset(
        dx: Float,
        dy: Float,
    ): RRect = RRect(RRectNative.nInset(nativePtr, dx, dy))

    fun outset(
        dx: Float,
        dy: Float,
    ): RRect = RRect(RRectNative.nOutset(nativePtr, dx, dy))
...
private object RRectNative {
    ...
    external fun nMakeRectXY(
        rect: FloatArray,
        xRad: Float,
        yRad: Float,
    ): Long

    external fun nMakeRectRadii(
        rect: FloatArray,
        radii: FloatArray,
    ): Long
