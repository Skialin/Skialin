...
class ColorSpace internal constructor(
    ptr: Long,
) : Managed(ptr, ColorSpaceNative::nRelease) {
    ...
    companion object {
        ...
        fun makeRGB(
            transferFn: FloatArray,
            toXyzD50: FloatArray,
        ): ColorSpace = ColorSpace(ColorSpaceNative.nMakeRGB(transferFn, toXyzD50))

        /** Null for an invalid or unsupported combination of code points. */
        fun makeCICP(
            primaries: CicpPrimaries,
            transferFn: CicpTransferFn,
        ): ColorSpace? = ColorSpaceNative.nMakeCICP(primaries.code, transferFn.code).takeIf { it != 0L }?.let { ColorSpace(it) }
        ...
        fun deserialize(bytes: ByteArray): ColorSpace? = ColorSpaceNative.nDeserialize(bytes).takeIf { it != 0L }?.let { ColorSpace(it) }
    }
}
...
private object ColorSpaceNative {
    ...
    external fun nMakeRGB(
        transferFn: FloatArray,
        toXyzD50: FloatArray,
    ): Long

    external fun nMakeCICP(
        primaries: Int,
        transferFn: Int,
    ): Long
    ...
    external fun nGamutTransformTo(
        ptr: Long,
        dstPtr: Long,
    ): FloatArray
