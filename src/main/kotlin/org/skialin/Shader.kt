...
class Shader internal constructor(ptr: Long) : Managed(ptr, ShaderNative::nRelease) {
    ...
    companion object {
        ...
        fun makeLinearGradient(
            p0: Point, p1: Point, colors: IntArray, positions: FloatArray? = null,
            tileMode: TileMode = TileMode.CLAMP, localMatrix: Matrix33? = null,
        ): Shader? {
        ...
        fun makeRadialGradient(
            center: Point, radius: Float, colors: IntArray, positions: FloatArray? = null,
            tileMode: TileMode = TileMode.CLAMP, localMatrix: Matrix33? = null,
        ): Shader? {
        ...
        fun makeTwoPointConicalGradient(
            start: Point, startRadius: Float, end: Point, endRadius: Float,
            colors: IntArray, positions: FloatArray? = null,
            tileMode: TileMode = TileMode.CLAMP, localMatrix: Matrix33? = null,
        ): Shader? {
            val ptr = ShaderNative.nMakeTwoPointConicalGradient(
                start.x, start.y, startRadius, end.x, end.y, endRadius, colors, positions, tileMode.ordinal, localMatrix?.values,
            )
            return if (ptr == 0L) null else Shader(ptr)
        }
        ...
        fun makeSweepGradient(
            center: Point, colors: IntArray, positions: FloatArray? = null,
            startAngle: Float = 0f, endAngle: Float = 360f,
            tileMode: TileMode = TileMode.CLAMP, localMatrix: Matrix33? = null,
        ): Shader? {
            val ptr = ShaderNative.nMakeSweepGradient(center.x, center.y, startAngle, endAngle, colors, positions, tileMode.ordinal, localMatrix?.values)
            return if (ptr == 0L) null else Shader(ptr)
        }
    }
}
...
private object ShaderNative {
    ...
    external fun nMakeLinearGradient(x0: Float, y0: Float, x1: Float, y1: Float, colors: IntArray, positions: FloatArray?, tileMode: Int, localMatrix: FloatArray?): Long
    external fun nMakeRadialGradient(cx: Float, cy: Float, radius: Float, colors: IntArray, positions: FloatArray?, tileMode: Int, localMatrix: FloatArray?): Long
    external fun nMakeTwoPointConicalGradient(
        startX: Float, startY: Float, startRadius: Float, endX: Float, endY: Float, endRadius: Float,
        colors: IntArray, positions: FloatArray?, tileMode: Int, localMatrix: FloatArray?,
    ): Long
    external fun nMakeSweepGradient(cx: Float, cy: Float, startAngle: Float, endAngle: Float, colors: IntArray, positions: FloatArray?, tileMode: Int, localMatrix: FloatArray?): Long
}
