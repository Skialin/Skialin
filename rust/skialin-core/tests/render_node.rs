...
use skialin_core::{color, AlphaType, ColorType, ImageInfo, Paint, Rect, RenderNode, RenderNodeContext, Surface};
...
fn clip_confines_content() {
    ...
    node.set_bounds(Rect::new(0.0, 0.0, 16.0, 16.0));
    node.set_clip(true);
    node.set_clip_rect(Some(Rect::new(0.0, 0.0, 8.0, 16.0)), ClipOp::Intersect);
    {
        let mut canvas = node.begin_recording();
        let mut paint = Paint::new();
        paint.set_color(color::RED);
