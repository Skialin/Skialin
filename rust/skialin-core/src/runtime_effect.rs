use crate::{sys, Blender, ColorFilter, Matrix, Shader};
...
impl RuntimeEffect {
    ...
    pub fn make_for_blender(sksl: &str) -> Result<Self, String> {
        let mut error: *mut sys::SkData = std::ptr::null_mut();
        let effect = unsafe { sys::skialin_bridge_RuntimeEffect_MakeForBlender(sksl.as_ptr().cast(), sksl.len(), &mut error) };
        Self::from_result(effect, error)
    }
    ...
    pub fn make_blender(&self, uniforms: &[u8], children: &[&Shader]) -> Option<Blender> {
        let child_ptrs: Vec<*mut sys::SkShader> = children.iter().map(|c| c.0).collect();
        unsafe { Blender::from_raw(sys::skialin_bridge_RuntimeEffect_makeBlender(self.0, uniforms.as_ptr(), uniforms.len(), child_ptrs.as_ptr(), child_ptrs.len())) }
    }
}
