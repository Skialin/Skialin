...
/// Wraps a GrDirectContext (Ganesh + OpenGL, Vulkan, Direct3D 12 on Windows, or Metal on macOS). The caller must make
...
impl DirectContext {
    ...
    /// Wraps a caller-created `id<MTLDevice>`/`id<MTLCommandQueue>` (as raw pointers). Each is
    /// retained for as long as the context needs it; the caller keeps its own reference. Always
    /// `None` off macOS.
    pub fn new_metal(device: *mut std::ffi::c_void, queue: *mut std::ffi::c_void) -> Option<Self> {
        let ptr = unsafe { sys::skialin_bridge_DirectContext_MakeMetal(device, queue) };
        (!ptr.is_null()).then_some(DirectContext(ptr, None))
    }
