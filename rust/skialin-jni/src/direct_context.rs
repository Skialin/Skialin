...
#[no_mangle]
pub extern "system" fn Java_org_skialin_DirectContextNative_nMakeD3D(
    _env: JNIEnv,
    _class: jni::objects::JClass,
    adapter: jlong,
    device: jlong,
    ...
    match DirectContext::new_d3d(adapter as _, device as _, queue as _, protected_context != 0) {
        Some(context) => box_ptr(context),
        None => 0,
    }
}
...
#[no_mangle]
pub extern "system" fn Java_org_skialin_DirectContextNative_nMakeMetal(_env: JNIEnv, _class: jni::objects::JClass, device: jlong, queue: jlong) -> jlong {
    match DirectContext::new_metal(device as _, queue as _) {
        Some(context) => box_ptr(context),
        None => 0,
    }
}
